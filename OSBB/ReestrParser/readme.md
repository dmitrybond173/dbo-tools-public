...
В выписке из "приватбанка" довольно много операций группируются и пемечаются как 
"Оплата згiдно реєстру № @xxxxxxxxx вiд nnnnnnnn"
И там внутри этого "рестра" может быть много платежей.
...
__Примечание__: kvRef - это ссылка на номер квартины - специфично для проведения ревизии...
...
Будем считать, что первая версия не имеет правильных primary key - у нас все что распарсилось, будет вставляеться в базу без проблем.
...
Нет времени разбираться, проведение ревизии и создание инструментов для нее не являетмся моей основаной работой...
...
Т.е. после парсинга оно сверяет по каждому файлу - а соспадает ли сумма по операциям с итоговой суммой по этому реестру.
