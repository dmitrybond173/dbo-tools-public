...
namespace Plugin.CslmonSyncVisualization
{
    public class LogFactsExtractorPlugin
    {
        public LogFactsExtractorPlugin()
        {
            ...
            this.info["Name"] = "Cslmon-Sync-Visualization";
            this.info["LogType"] = "CSLMON-Sync";
            this.info["PerProject"] = "true";
            this.info["VisualizationTypes"] = "bitmap";

            this.SyncNames = new List<string>();
            this.SelectedSyncNames = new List<string>();
            ...
            this.visualParams["SyncNames"] = this.SyncNames;
            this.visualParams["SelectedSyncNames"] = this.SelectedSyncNames;
        ...
        public List<string> SelectedSyncNames;
        ...
        public void Activate(DbConnection db, DbProviderFactory factory, string pTableName, int pProjId, int pLogId)
        {
            ...
            using (DbCommand cmd = db.CreateCommand())
            {
                ...
                this.SyncNames.Clear();
                cmd.CommandText = string.Format(
                    "SELECT DISTINCT syncId FROM {0} WHERE projectId = {1}",
                    pTableName, pProjId
                    );
                using (DbDataReader dr = cmd.ExecuteReader())
                {
                    while (dr.Read())
                    {
                        this.SyncNames.Add(dr[0].ToString());
                    }
                }
                Trace.WriteLine(string.Format("CslmonCallsVisualization.: syncNames= {0}", CollectionUtils.Join(this.SyncNames, ";")));

                if (!this.customParams.TryGetValue("MainForm", out obj))
                    obj = null;

                // === SHOW Params Selection UI ===
                if (!FormCslmonSyncVisParams.Execute((Form)obj, this.visualParams)) return;
