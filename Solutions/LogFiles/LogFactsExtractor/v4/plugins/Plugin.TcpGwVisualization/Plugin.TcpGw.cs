...
namespace Plugin.TcpGwVisualization
{
    public class LogFactsExtractorPlugin
    {
        public LogFactsExtractorPlugin()
        {
            ...
            this.info["Name"] = "TcpGwVisualization";
            this.info["LogType"] = "TCP-GW-Service";
            this.info["PerProject"] = "false";
            ...
            this.visualParams["VisualizationType"] = "bitmap";
        ...
        {
            Trace.WriteLine(string.Format("TcpGwVisualization.Activate( (db), (factory), tab={0}, projId={1}, logId={2} )", 
                pTableName, pProjId, pLogId));

            this.VisualFileID = string.Format("VisualFilename_p{0}_f{1}", pProjId, pLogId);
            ...
            using (DbCommand cmd = db.CreateCommand())
            {
                cmd.CommandText = string.Format(
                    "SELECT min(logtime), max(logtime) FROM {0} WHERE projectId = {1} {2}",
                    pTableName, pProjId, (pLogId > 0 ? string.Format("AND logId = {0}", pLogId) : "")
                    );
                Trace.WriteLine(string.Format("TcpGwVisualization.ExecSQL: {0}", cmd.CommandText));
                ...
                { 
                    if (dr.Read())
                    {
                        minTs = dr[0].ToString();
                        maxTs = dr[1].ToString();
                        Trace.WriteLine(string.Format("TcpGwVisualization: minTs={0}, maxTs={1}", minTs, maxTs));
                    }
                }
                ...
                if (!FormTcpGwVisualizationParams.Execute((Form)obj, this.visualParams)) return;
                ...
                using (DbDataAdapter da = factory.CreateDataAdapter())
                {
                    ...
                    Trace.WriteLine(string.Format("TcpGwVisualization.Resultset: {0} cols x {1} rows", this.sourceData.Columns.Count, this.sourceData.Rows.Count));
                }
